<?php
/**
 * SHALOM FACTURA - API Reenviar Nota de Crédito
 */
...
if (!auth()->can('notas_credito.crear')) {
    Helpers::json(['success' => false, 'message' => 'Sin permisos para reenviar notas de crédito'], 403);
}
...
try {
    ...
    $result = $notaModel->reenviar((int) $input['id']);
    Helpers::json($result);
} catch (\Exception $e) {
    Helpers::json(['success' => false, 'message' => 'Error al reenviar la nota de crédito: ' . $e->getMessage()], 500);
}

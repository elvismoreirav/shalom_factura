...
$this->addElement($iEl, 'baseImponible', $this->formatNumber($imp['base_imponible']));
    ...
    public function generarNotaCredito(array $data): string {
        $this->dom = new DOMDocument('1.0', 'UTF-8'); $this->dom->formatOutput = true;
        $root = $this->dom->createElement('notaCredito'); $root->setAttribute('id', 'comprobante'); $root->setAttribute('version', '1.1.0');
        $this->dom->appendChild($root); return $this->dom->saveXML();
    }
